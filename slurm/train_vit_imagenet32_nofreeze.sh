#!/bin/bash
# ViT-small, no-freeze baseline, Downsampled ImageNet (32x32) cluster config.
# Submit from the repo root: sbatch slurm/train_vit_imagenet32_nofreeze.sh
#
# --time is a rough placeholder, not measured: config_vit_small_imagenet32_cluster.yaml's
...
#SBATCH --job-name=vit_imagenet32_nofreeze
...
python src/train.py \
  --model vit_small \
  --config src/configs/config_vit_small_imagenet32_cluster.yaml \
  --disable-freeze
